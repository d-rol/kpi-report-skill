...
def delay_cause(verdict):
    """Природа задержки по унаследованному нарушению, а не только её источник.

    «Из общей очереди» отвечает на вопрос ОТКУДА пришло обращение, но не на
    вопрос ПОЧЕМУ оно столько ждало — а руководителю нужен второй. Ночная пачка
    ждала открытия смены, обращение в разбор ночи ждало, пока разгребут ночь,
    дневное — что его кто-нибудь возьмёт. Это три разные причины, и лечатся они
    по-разному.

    Живёт здесь, а не в рендере, потому что рендеров два (чат и HTML) и
    формулировка в них обязана быть одна.
    """
    # Передача между операторами — причина сама по себе, вид прихода вторичен:
    # обращение уже было чьим-то, и ждало оно не очереди, а конкретного человека.
    if verdict.get("neglected_by"):
        return verdict.get("reason") or "передано от другого оператора"
    kind = verdict.get("arrival")
    if kind == NIGHT:
        return ("пришло в нерабочее время — часы SLA стартовали с открытием "
                "смены вместе со всей ночной пачкой")
    if kind == DRAIN:
        return "пришло, пока разбирали ночную очередь"
    return "висело в общей очереди без ответственного"

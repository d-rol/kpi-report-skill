...
def load_lines(load):
    ...
    if not load.get("available"):
        ...
        ref = reference_lines(load.get("reference"))
        tail = ("Проценты выше от этого не меняются, но контекста «поток был "
                "обычный или нет» у отчёта пока нет.")
        if ref:
            # Норма ниже отвечает на абсолютный вопрос, поэтому «контекста нет»
    ...
    return [line] + reference_lines(load.get("reference"))


def reference_lines(ref):
    """Норма нагрузки — отдельной строкой от скользящей базы, не в одной фразе.

    База говорит «необычен ли поток относительно недавнего», норма — «тяжёлый ли
    период вообще». Это разные утверждения, и слитые в одно предложение они
    читаются как одно, причём более уверенное, чем каждое по отдельности.

    Нормы может не быть (её не задали) — тогда молчим: строка нагрузки сама по
    себе ничего про «обычность» не утверждает, в отличие от отсутствующей базы.
    """
    if not ref:
        return []
    corridor = ""
    ...
    verdict = (f" → период **{ref['verdict']}**" if ref.get("verdict")

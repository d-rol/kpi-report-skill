...
# ⚠ Граница «личное / унаследованное» стоит на ДВУХ ногах, и вторая — не в коде.
# Удержание считается от первого назначения ответственного, а назначение обычно
# делает правило Omnidesk «назначить, если обращение больше N без ответственного».
# Его интервал и задаёт потолок «ничьего» времени, которое по построению не идёт
# в вину. Уменьшите интервал правила — часть унаследованных станет личными, и
# наоборот, причём в коде при этом не изменится ничего. Проверьте, какие правила
# назначают ответственного у вас. Через API они не читаются (403), но имя
# сработавшего правила видно в changelog обращения (событие `rules`).

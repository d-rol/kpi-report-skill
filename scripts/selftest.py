...
# --------------------------------------------------------------------------
section("Ночная очередь и её разбор")
# --------------------------------------------------------------------------
# Суть механики: ночью смены нет, часы SLA по всей ночной пачке стартуют в 10:00
# ОДНОВРЕМЕННО. Уложиться может только первая горстка, остальные просрочены по
# построению. Плюс второй эффект: пока разбирают ночь, приходят новые обращения,
# и ответить на них раньше физически нельзя — но выглядят они как обычные
# дневные просрочки. Момент разбора считается по данным, а не константой.
D0 = dt.datetime(2026, 7, 20, tzinfo=MSK)          # понедельник


def at(day, h, m=0):
    return D0 + dt.timedelta(days=day, hours=h, minutes=m)


# Ночная пачка: пришли в 03:00 и 06:00, часы у обеих стартуют в 10:00.
check("часы ночного стартуют с открытием смены",
      backlog.clock_start(at(0, 3)), at(0, 10))
check("часы дневного стартуют сразу", backlog.clock_start(at(0, 14)), at(0, 14))
# Пришло после закрытия — ждёт СЛЕДУЮЩЕГО дня, а не сегодняшнего 10:00.
check("пришло после закрытия — ждёт следующего дня",
      backlog.clock_start(at(0, 23)), at(1, 10))

# День: две ночные (ответ через 20 и 50 мин после открытия) и три дневные.
day_cases = [
    (at(0, 3), 10.0),      # ночная, отвечена в 10:10 — успели в SLA
    (at(0, 6), 50.0),      # ночная, отвечена в 10:50 -> разбор кончился тут
    (at(0, 10, 30), 25.0),  # пришла ВО ВРЕМЯ разбора, просрочка
    (at(0, 11), 40.0),     # пришла ПОСЛЕ разбора, просрочка
    (at(0, 15), 5.0),      # обычная, в норме
]
ends = backlog.drain_end(day_cases)
check("разбор кончается на последнем ночном", ends[at(0, 10).date()], at(0, 10, 50))
check("пришло во время разбора", backlog.arrival_kind(at(0, 10, 30), ends), backlog.DRAIN)
check("пришло после разбора", backlog.arrival_kind(at(0, 11), ends), backlog.NORMAL)
check("ночное остаётся ночным", backlog.arrival_kind(at(0, 3), ends), backlog.NIGHT)

s = backlog.summary(day_cases, 15.0)
check("ночных посчитали", s["kinds"]["night"]["cases"], 2)
check("во время разбора посчитали", s["kinds"]["drain"]["cases"], 1)
check("обычных посчитали", s["kinds"]["normal"]["cases"], 2)
# 50 мин > 15 => ночная просрочка; 25 мин в разборе; 40 мин в обычное время.
check("просрочки разнесены по видам",
      [s["kinds"][k]["violations"] for k in ("night", "drain", "normal")], [1, 1, 1])
check("доля очереди в просрочках", s["queue_share"], round(2 / 3, 3))

# Обращения без ответа не задают момент разбора: одно забытое ночное растянуло
# бы окно разбора на всю смену и утопило в нём весь день.
with_lost = day_cases + [(at(0, 4), None)]
check("без ответа окно разбора не растягивает",
      backlog.drain_end(with_lost)[at(0, 10).date()], at(0, 10, 50))

# Ночную очередь разобрали ДО открытия смены — окна разбора нет вообще.
early = [(at(1, 3), -30.0), (at(1, 12), 40.0)]
e_ends = backlog.drain_end(early)
check("разобрано до открытия: дневное не считается разбором",
      backlog.arrival_kind(at(1, 12), e_ends), backlog.NORMAL)

# Рендер: пометка у случая и сводная строка.
check("пометка ночной очереди",
      report.arrival_note({"arrival": backlog.NIGHT}), " · ночная очередь")
check("без пометки — пусто", report.arrival_note({"held_min": 40}), "")
line = "\n".join(report.backlog_lines(s))
check("сводка называет долю очереди", "(67%)" in line, True)
check("сводка объясняет механику", "одновременно" in line, True)
# В личном отчёте раздела нет — рендер обязан промолчать, а не упасть.
check("ночной очереди нет (personal): строк нет", report.backlog_lines(None), [])
check("нет просрочек — нет и раздела",
      report.backlog_lines(backlog.summary([(at(0, 15), 5.0)], 15.0)), [])
...
# Рабочее окно живёт в ДВУХ модулях независимо (третий, backlog, переиспользует
# окно аудита и своих порогов не имеет — потому и не сканируется выше). Если
# развести копии, удержание в аудите, нагрузка на оператора и ночная очередь
# начнут считаться по разным суткам, и все три цифры останутся правдоподобными.
